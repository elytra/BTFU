...
case class BTFUConfig(maxBackups: Int, disablePrompts: Boolean, cmds: BTFUNativeCommands,
                               ...
                               dateFormat: SimpleDateFormat, notDateFormat: SimpleDateFormat, numberInactiveBackups: Int, c: ConfWrapper) {
...
object BTFUConfig {
  ...
  def apply(c: ConfWrapper): BTFUConfig = {
    ...
    val conf = BTFUConfig(
      ...
      c.getInt("BTFU", "number of backups to keep after no player is online, set to -2 to keep all", -2),
      c
    )
    c.checkAndSave()
    conf
