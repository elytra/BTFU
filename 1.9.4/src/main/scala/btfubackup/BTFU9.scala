...
object LogWrapper9 extends LogWrapper {
  override def debug(s: String): Unit = BTFU9.logger.debug(s)
  override def err(s: String): Unit = BTFU9.logger.error(s)
  override def warn(s: String): Unit = BTFU9.logger.warn(s)
  override def warn(s: String, e: Throwable): Unit = BTFU9.logger.warn(s, e)
}

@Mod(modid = "btfu", version = "1", name = "BTFU", modLanguage = "scala", acceptableRemoteVersions="*") object BTFU9 extends BTFU(LogWrapper9) {
  var logger:Logger = _
  ...
  def init(e: FMLPreInitializationEvent): Unit = {
    ...
    cfg = BTFUConfig(ConfWrapper9(e.getSuggestedConfigurationFile))
    ...
    val handler = new Object {
      ...
      def onServerTick(event: ServerTickEvent): Unit = {
        if (event.phase == TickEvent.Phase.START) {
          WorldSavingControl9.mainThreadTick()
          BTFUPerformer.tick()
        }
      }
    }
