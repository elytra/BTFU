...
object LogWrapper15 extends LogWrapper {
  override def err(s: String): Unit = BTFU15.logger.error(s)
  override def warn(s: String): Unit = BTFU15.logger.warn(s)
  override def warn(s: String, e: Throwable): Unit = BTFU15.logger.warn(s, e)
}

@Mod(modid = "btfu", version = "1", name = "BTFU", modLanguage = "scala", acceptableRemoteVersions="*") object BTFU15 extends BTFU(LogWrapper15) {
  var logger:Logger = _
  ...
  def init(e: FMLPreInitializationEvent): Unit = {
    ...
    cfg = BTFUConfig(ConfWrapper15(e.getSuggestedConfigurationFile))
